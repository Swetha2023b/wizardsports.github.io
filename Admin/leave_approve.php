...
<div id="wrapper">
  ...
  <!-- <form class="navbar-form navbar-right">
		...
		<div class="navbar-default sidebar" role="navigation">
		  ...
		  <?php
    $query = "SELECT * from tbl_leave ORDER BY date DESC";
     ...
     <thead>
      ...
      <?php  while ($row = mysqli_fetch_array($result)) { ?>
                    <tr>
                        ...
                        <td>
                            <?php 
                                $querys = "SELECT * from tbl_coach_register WHERE login_id = '{$row['login_id']}'";
                                $results = mysqli_query($db,$querys);
                                if ($results->num_rows > 0) {
                                    $rows = $results->fetch_assoc();
                                    echo $rows['teach_name'];
                                } else {
                                    echo "No result";
                                }
                            ?>
						...
						<?php if ($row['status'] == 'Not Approved') { ?>
							<button class="btn btn-primary" style="color: white;"><a href="leave_confirm.php?id=<?php echo $row['leave_id'];?>">Pending</button>
						<?php } elseif ($row['status'] == 'Approved') { ?>
							<button class="btn btn-primary" style="background-color: lightgreen; color: white;" disabled>Approved</button>
						<?php } ?>
						</td>                       
					</tr>   
        <?php } ?>
